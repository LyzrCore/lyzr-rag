...
def set_global_tokenizer(tokenizer: Union[Tokenizer, Callable[[str], list]]) -> None:
    ...
    if isinstance(tokenizer, Tokenizer):
        lyzr_rag.global_tokenizer = tokenizer.encode
    else:
        lyzr_rag.global_tokenizer = tokenizer


def get_tokenizer() -> Callable[[str], List]:
    import lyzr_rag

    if lyzr_rag.global_tokenizer is None:
        tiktoken_import_err = (
    ...
    assert lyzr_rag.global_tokenizer is not None
    return lyzr_rag.global_tokenizer
...
def get_cache_dir() -> str:
    """Locate a platform-appropriate cache directory for lyzr_rag,
    ...
    # Linux, Unix, AIX, etc.
        ...
        path = Path("/tmp/lyzr_rag")

    # Mac OS
    elif sys.platform == "darwin":
        path = Path(os.path.expanduser("~"), "Library/Caches/lyzr_rag")
    ...
    # Windows (hopefully)
        ...
        path = Path(local, "lyzr_rag")

    if not os.path.exists(path):
        os.makedirs(
            path, exist_ok=True
        )  # prevents https://github.com/jerryjliu/lyzr_rag/issues/7362
    return str(path)
...
# Sample text from lyzr_rag's readme
SAMPLE_TEXT = """
Context

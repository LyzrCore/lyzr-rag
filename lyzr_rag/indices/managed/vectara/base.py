...
from lyzr_rag.data_structs.data_structs import IndexDict, IndexStructType
from lyzr_rag.indices.managed.base import BaseManagedIndex, IndexType
from lyzr_rag.schema import BaseNode, Document, MetadataMode, TextNode
...
class VectaraIndex(BaseManagedIndex):
    ...
    def insert_file(
        ...
        **insert_kwargs: Any,
        ...
        This method provides a way to use that API in lyzr_rag.
    ...
    def as_query_engine(self, **kwargs: Any) -> BaseQueryEngine:
        if kwargs.get("summary_enabled", True):
            from lyzr_rag.indices.managed.vectara.query import VectaraQueryEngine

            kwargs["summary_enabled"] = True
            retriever = self.as_retriever(**kwargs)
            return VectaraQueryEngine.from_args(retriever, **kwargs)  # type: ignore
        else:
            from lyzr_rag.query_engine.retriever_query_engine import (

...
if TYPE_CHECKING:
    from lyzr_rag.bridge.langchain import BasePromptTemplate as LangchainTemplate
...
from lyzr_rag.prompts.utils import get_template_vars
...
class LangchainPromptTemplate(BasePromptTemplate):
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError(
                "Must install `lyzr_rag[langchain]` to use LangchainPromptTemplate."
            )
        if selector is None:
            if template is None:
